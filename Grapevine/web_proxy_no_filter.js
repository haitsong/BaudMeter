...
// cache domain in the requests thru the proxy, set a limit to prevent it bloom up the memory
var domain_match_list_size = 20000;
...
function CacheDomains(domain, callback){
    ...
    if(Object.keys(domain_match_list).length > domain_match_list_size){
        domain_match_list = {};
    }

    if(callback != null){
        process.nextTick(function(){callback(null, domain_match_list);});
    }
}
...
function httpUserRequest( userRequest, userResponse ) {
    ...
    // cache domain in user request.
    CacheDomains(hostport[0], null);
